...
try:
    ...
    if df is not None:
        ...
        p_names = ["หินใหญ่(ลบ.ม.)", "หินย่อย(ลบ.ม.)", "ทรายหยาบ(ลบ.ม.)", "ปูนซีเมนต์(ถุง)", "หินคลุก(ลบ.ม.)", "เหล็กเส้นเสริมคอนกรีต(ตัน)", "ลวดผูกเหล็กเสริม(กก.)"]
        ...
        if col_in3.button("➕ เพิ่มรายการ", use_container_width=True):
            if q_val is not None and q_val > 0:
                ...
                m_map = {"หินใหญ่(ลบ.ม)": 2, "หินย่อย(ลบ.ม)": 4, "ทรายหยาบ(ลบ.ม)": 6, "ปูนซีเมนต์(ถุง)": 8, "หินคลุก(ลบ.ม)": 10, "เหล็กเส้นเสริมคอนกรีต(ตัน)": 12, "ลวดผูกเหล็กเสริม(กก.)": 14}
                temp_details = {}

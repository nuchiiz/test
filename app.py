...
try:
    ...
    if df is not None:
        ...
        if st.session_state.calc_history:
            ...
            # ---------------------------------------------------------
        # 4. สรุปผล (ฉบับปรับปรุง: จัดกึ่งกลาง 100%)
        # ---------------------------------------------------------
        if st.session_state.calc_history:
            st.divider()
            
            # หัวข้อส่วนที่ 4 จัดกึ่งกลาง
            st.markdown("<h2 style='text-align: center;'>📊 4. สรุปผลและเปรียบเทียบแผนงาน</h2>", unsafe_allow_html=True)
            
            # คำนวณยอดรวม
            totals = {k: 0.0 for k in p_names}
            for item in st.session_state.calc_history:
                for m_n, m_v in item['details'].items() if 'details' in item else item['รายละเอียด'].items():
                    if m_n in totals:
                        totals[m_n] += m_v

            # แสดงผล Metric แบบจัดกึ่งกลาง
            # ใช้ CSS ควบคุมการจัดวาง Metric
            st.markdown("""
                <style>
            ...
            for name in p_names:
                ...
                # กำหนดสถานะ
                status = "✅ ปกติ" if diff >= 0 else "⚠️ เกินแผน"
            ...
            # การจัดรูปแบบตารางให้ "ตัวหนังสืออยู่ตรงกลาง" และ "หัวตารางสวยงาม"
            ...
            styled_table = df_comp.style.format({
                ...
                'text-align': 'center',      # จัดเนื้อหาทุกช่องให้อยู่ตรงกลาง
                'vertical-align': 'middle'
            }).set_table_styles([
                {'selector': 'th', 'props': [('text-align', 'center'), ('background-color', '#f0f2f6'), ('font-weight', 'bold')]} # จัดหัวตาราง
            ]).applymap(highlight_diff, subset=["ส่วนต่าง (+เหลือ/-เกิน)"])
